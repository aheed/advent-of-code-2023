...
def extrapolate(history: list[int]) -> int:
    ...
    while True:
        if collection_index == 0:
            break
        new_value = (diff_collections[collection_index-1])[-1] + (diff_collections[collection_index])[-1]
        diff_collections[collection_index-1].append(new_value)
        collection_index = collection_index - 1
    
    #print(diff_collections)

    return diff_collections[0][-1]
